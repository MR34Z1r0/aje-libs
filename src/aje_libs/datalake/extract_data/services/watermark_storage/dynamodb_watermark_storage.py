...
class DynamoDBWatermarkStorage(IWatermarkStorage):
    ...
    def cleanup_table_watermarks(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            # Construir el watermark key pattern
            # Formato: project_name#table_name#column_name
            watermark_prefix = f"{self.team}#{self.data_source}#{self.endpoint_name}#{(table_name or '').lower()}#"
            
            self.logger.info(
                f"🧹 Limpiando watermarks - "
                f"DynamoDB table: {self.table_name}, "
                f"project_name: {self.project_name}, "
                f"table_name (STAGE_TABLE_NAME): {table_name}, "
                f"watermark_prefix (buscará): {watermark_prefix}"
            )
            self.logger.info(
            ...
            if not watermarks:
                self.logger.info(f"✅ No se encontraron watermarks para limpiar")
                result['success'] = True
                result['details'] = "No se encontraron watermarks"
                return result
            
            # Eliminar cada watermark
            deleted_count = 0
            for watermark in watermarks:
                try:
                    watermark_key = watermark.get('WATERMARK_KEY')
                    ...
                    error_msg = f"Error eliminando watermark {watermark.get('WATERMARK_KEY', 'unknown')}: {str(e)}"
            ...
            error_msg = f"Error durante limpieza de watermarks: {str(e)}"
